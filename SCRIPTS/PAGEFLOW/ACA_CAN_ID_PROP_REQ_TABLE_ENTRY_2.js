...
try {
    ...
    var cloneSite = AInfo["Clones cut/grown on-site"];
    var seedSite = AInfo["Seeds produced/started on-site"];
    if (cloneSite == "Yes" || seedSite == "Yes") {

        var propagationCounter;
        if (typeof(PROPAGATION) == "object") {
            propagationCounter = PROPAGATION.length;
        }
        if (propagationCounter < 1) {
            messageList += "You must enter at least one row in the following table: " + "Propagation" + br;
        }
    }
