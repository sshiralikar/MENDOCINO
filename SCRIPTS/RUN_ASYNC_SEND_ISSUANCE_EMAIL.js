    ...
    if (contactResult.getSuccess()) {
        ...
        for (var i in capContacts) {
            if(capContacts[i].getPeople().getContactType() == "Applicant"
                || capContacts[i].getPeople().getContactType() == "Authorized Agent")
            {
                ...
                if(hm[capContacts[i].getPeople().getEmail() + ""] != 1) {
                    sendEmail("no-reply@mendocinocounty.org", capContacts[i].getPeople().getEmail() + "", "", "CAN_PERMIT_ISSUANCE", params, VRFiles, capId);
