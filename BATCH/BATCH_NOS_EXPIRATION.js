/*------------------------------------------------------------------------------------------------------/
| Program: BATCH_NOS_EXPIRATION.js
| Trigger: Batch
| Client: MENDOCINO
| Version 1.0 5/10/2023
| Author: Shashank Shiralikar
| This batch script will run daily @11:35 PM.
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
function mainProcess() {
    try {
        ...
        var recordListResult = aa.cap.getCapIDsByAppSpecificInfoField("NOAS Expiration Date", String(dateCalc));
        if (!recordListResult.getSuccess())
            ...
            for (var j in recArray) {
                ...
                var fileDateStr = fileDate.getMonth() + "/" + fileDate.getDayOfMonth() + "/" + fileDate.getYear();
                var currDate = new Date(fileDateStr);

                var ph1Date1 = new Date("12/30/2016");
                var ph1Date1Str = (ph1Date1.getMonth() + 1) + "/" + ph1Date1.getDate() + "/" + ph1Date1.getFullYear();

                var ph1Date2 = new Date("10/4/2019");
                var ph1Date2Str = (ph1Date2.getMonth() + 1) + "/" + ph1Date2.getDate() + "/" + ph1Date2.getFullYear();


                targetAppType = cap.getCapType();     //create CapTypeModel object
                targetAppTypeString = targetAppType.toString();
                if(targetAppTypeString.split("/")[2] == "Application" && (currDate > ph1Date1) && (currDate < ph1Date2))
                {
                        ...
                        for (var i in capContacts) {
                            if(matches(capContacts[i].getPeople().getContactType(),"Applicant","Authorized Agent"))
                            {
                                ...
                                if(hm[applicantEmail+""] != 1) {
                                    sendEmail("no-reply@mendocinocounty.org", applicantEmail, "", "CAN_NOS_EXPIRED", params, null, capId);
                                    hm[applicantEmail+""] = 1;
                                }
                            }
                        }
                    ...
                    var acaUrl = String(lookup("ACA_CONFIGS", "ACA_SITE")).split("/Admin")[0];
                    addParameter(params, "$$acaRecordUrl$$", acaUrl);
                    sendEmail("no-reply@mendocinocounty.org", lookup("NOTIFICATION_TEMPLATE_INFO_EXTERNAL_AGENCIES","EXTERNAL_AGENCIES")+"", "", "CAN_NOS_EXPIRED", params, null, capId);
                }
            }
        }
        var numberOfDays = 0;
