...
function UpdateFields(rules) {
	...
	if (!isEmptyOrNull(updateAppName)) {
        ...
        var customField = updateAppName.customField ;
		...
		if(!isEmptyOrNull(customField))
		{
			if(customField.indexOf(".") > -1) 
				{
		...
		var customFieldValue = getAppSpecific(customField) ;
		vNewAppName = customFieldValue ;
		}
		editAppName(vNewAppName,itemCap);	
		logDebug("Edit application name value to '" +vNewAppName +"'")
	}
	var updateShortNotes =rules.updateShortNotes ;
	if (!isEmptyOrNull(updateShortNotes))
		{
		customFieldName= updateShortNotes.customField ;
		if(!isEmptyOrNull(customFieldName))
		{       
		 var customFieldValue = getAppSpecific(customFieldName) ; 
			if( !isEmptyOrNull(customFieldValue) )
				{
					...
					cd.setShortNotes(customFieldValue) ;
					aa.cap.editCapDetail(cd);
					logDebug("Update Record-Short Notes field value to:'" + customFieldValue+ "' , the source value from ASI field:'" +customFieldName + "'.") ;
					}	
					else
					{
						logDebug("Error in aa.cap.getCapDetail API. Message = " + cdScriptObjResult.getErrorMsg());
					}
				}
			else 
			{
			logDebug("ASI field name:'" +customFieldName + "' is not exist to update Record-Short Notes field." );
			}
		}
		}
}
	...
	if (valueSource && valueSource.trim() == "") {
		...
		if (stdValues.length == 2) {
			...
			if (result != null) {
				...
				logDebug("WARNING: Failed to get a value's description for standard choice name: " + stdValues[0] + " , Value: " +stdValues[1] );
				return "";
			}
		}
