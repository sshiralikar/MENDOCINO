...
Description : JSON must contain :
	"Module/Type/Subtype/Category" = 4 level record structure parent
		"Rule set" - SubParent.  Event name plus rules.  Currently supports only wftTask/wfStatus or inspType/inspResult. example: "WorflowTaskUpdateAfter/wfTask/wfStatus" 
			...
			"remoteInspection": /optional/ if set to true on InspectionScheduleAfter a lookup will be performed on the REMOTE_INSPECTIONS_URLS Standard 
			to get the inspectors Web Conference URL 
...

{
  "Marijuana/Enforcement/Complaint/NA": {
    "ApplicationSpecificInfoUpdateAfter": [
      {
        ...
        "action": {
		  ...
		  "useDepartmentInformationStandardChoice": true, Read department info from 'DEPARTMENT_INFORMATION' other else will read the department info from 'RPT_CONFIG' Choice, in case the 'RPT_CONFIG' is active 
        },
        "postScript": ""
      }
    ]
	...
	$$inspSchedTimeStart$$    (inspection schedule events only)
	$$inspSchedTimeEnd$$    (inspection schedule events only)
	$$InspectorName$$    (inspection schedule events only)
	$$InspectorPhoneNumber$$    (inspection schedule events only)
	$$InspectorEmail$$
	$$inspectorWebConferenceURL$$   (inspection schedule events only)
...
function sendContactEmails(itemCapId, recordSettings, parameters) {
	...
	var rRemoteInspection = handleUndefined(recordSettings.action.remoteInspection, false);
	var rInspectionUnitNumber = handleUndefined(recordSettings.criteria.inspectionUnitNumber, false);
	var rUseDepartmentInformationStandardChoice = handleUndefined(recordSettings.action.useDepartmentInformationStandardChoice, false);
	...
	if (!isEmptyOrNull(rNotificationTemplate)) {
		try {
			...
			if (contactObjArray.length > 0 || !isEmptyOrNull(rAdditionalEmailsTo)) {
				...
				if(!isRPT_CONFIG_StandardChoiceExist || rUseDepartmentInformationStandardChoice )
				...
				if (controlString.indexOf("InspectionSchedule") > -1 || controlString.indexOf("InspectionMultipleScheduleAfter") > -1) {
					if (inspId) {
					...
					if (pim.getScheduledTime()) addParameter(eParams, "$$inspSchedTimeStart$$", pim.getScheduledTime2() + " " + pim.getScheduledTime());
					if (pim.getScheduledEndTime()) addParameter(eParams, "$$inspSchedTimeEnd$$", pim.getScheduledEndTime2() + " " + pim.getScheduledEndTime());

					if (inspInspectorObj) {
						var InspectorPhoneNumber = inspInspectorObj.getPhoneNumber();
						var InspectorEmail = inspInspectorObj.getEmail();
						if (InspectorPhoneNumber) addParameter(eParams, "$$InspectorPhoneNumber$$", InspectorPhoneNumber);
						if (InspectorEmail) addParameter(eParams, "$$InspectorEmail$$", InspectorEmail);
					}
					
					if(rRemoteInspection){
						var enableRemoteInspection = false;
						
						if (!isEmptyOrNull(rInspectionUnitNumber)){

							if(!isEmptyOrNull(pact.getUnitNBR())){
								var inspUnitNumberUpper = pact.getUnitNBR().toString().toUpperCase();

								for(iUnit in rInspectionUnitNumber){
									var vInspectionUnitNumber = rInspectionUnitNumber[iUnit];
									if(inspUnitNumberUpper == vInspectionUnitNumber){
										enableRemoteInspection = true;
									}
								}
							}
						} 
						else {
							// If no criteria are added use the action.remoteInspection flag status
							enableRemoteInspection = true;
						}
						
						if(enableRemoteInspection){
							var inspRemoteURL = lookup("REMOTE_INSPECTIONS_URLS", inspInspector);

							if(!isEmptyOrNull(inspRemoteURL)) {
								addParameter(eParams, "$$inspectorWebConferenceURL$$",inspRemoteURL);
								logDebug("Send Contact Emails: Located Remote Inspection information for " + inspInspector + " URL: " + inspRemoteURL);
							}
							else{
								logDebug("Send Contact Emails: Unable to find " + inspInspector + " in Standard Choice REMOTE_INSPECTIONS_URLS for Remote Inspections" )
							}
						}
