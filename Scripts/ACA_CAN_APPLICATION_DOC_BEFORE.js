/*------------------------------------------------------------------------------------------------------/
| Program : ACA_CAN_APPLICATION_DOC_BEFORE.js
        ...
        for (x in r) {
            if(uploadedDocs[r[x].document] == undefined) {
                ...
                if (dr && ccr.length > 0 && showList && publicDisplayCond) {
                    message += "<li><span>" + dr.split("-")[1] + "</span>: " + publicDisplayCond.getPublicDisplayMessage() + "</li>";
                }

                if (dr && ccr.length > 0 && addConditions && !appHasConditionX(conditionType,null,dr,null)) {
