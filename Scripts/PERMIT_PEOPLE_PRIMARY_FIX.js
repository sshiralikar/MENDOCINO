...
function mainProccess() {
    ...
    for(var i in json) {
        try {
            ...
            for(var recIndex in ids) {
                var altId = ids[recIndex];
                if(!altId) {
                    continue;
                }
                var capId = aa.cap.getCapID(altId).getOutput();
                if(capId) {
                    var result = correctTransactionPrimary(capId, altId);
                    coll.push(result);
                }            
            }        
        } catch (err) {
            logDebug(i + " " + err + " " + err.lineNumber);
        }
    }
    aa.print("Updated " + coll.length);
    aa.print(JSON.stringify(coll));
}

function correctTransactionPrimary(itemCap, recordId) {
    ...
    for(var contactIndex in contacts) {
        ...
        if(primaryFlag != "Y") {
            capContact.setPrimaryFlag("Y")
            var editResult = aa.people.editCapContact(capContact);
            if(editResult.getSuccess()) {
                logDebug("Updated email on transactional");
                updateObj.name = String(capContact.fullName);
                updateObj.trans_id = String(transactionId);
                updateObj.updated = true;
            } else {
                updateObj.error = String("Failed to update primary: " + editResult.getErrorMessage() + " " + editResult.getErrorType());
                logDebug(updateObj.error);
            }
            break;
        }        
    }        
