    ...
    "WorkflowTaskUpdateAfter": [                                     
      {
        ...
        "action": {
          ...
          "issuedStatus": "Issued",
		  "expirationStatus" : "Active", the automation will check the value is one of the system values for Expiration , if not, the automation will check the 'issuedStatus' value is one of the system values for Expiration. 
          "copyCustomFields": [
            "ALL"
          ],
          "copyCustomTables": [
...
function licenseIssuance(itemCapId, recordSettings) {
	...
	var rExpirationStatus = handleUndefined(recordSettings.expirationStatus, false);
	...
	if (rParentLicense != "") {
		...
		if (rB1ExpResult != null && rB1ExpResult.getB1Expiration() != null) {
			...
			var expirationStatusValue= getExpirationStatusValue(rExpirationStatus, rIssuedStatus);
			if (!isEmptyOrNull(expirationStatusValue))
			{
				rB1ExpResult.setExpStatus(expirationStatusValue);
			}
 ...
 * copy 'Application Name' field from source cap Id to destination cap Id
  */
...
/**
 * returns expiration status value after checking the valid value on the JSON properties ('expirationStatus' and 'issuedStatus') values
 *
 * @param {string} ExpStatus   
 * @param {string} issuedStatus         
 * @returns expirationStatus 
 */
function getExpirationStatusValue(ExpStatus,issuedStatus)
{
	var stdChoiceItem = "LICENSE EXPIRATION STATUS";
	//This to enables the standard choice item to enables check the item values
	enableStandardChoiceOrCreateItem(stdChoiceItem);
	var isValidValue =false;
	if(!isEmptyOrNull(ExpStatus))
		{
		isValidValue=  checkStandardchoiceValueIsActive(stdChoiceItem,ExpStatus);
		if(isValidValue)
			{
			logDebug("The valid expiration status value ='" +ExpStatus+ "'");
			return ExpStatus; 
			}
		}
	if(!isEmptyOrNull(issuedStatus))
		{
		isValidValue=  checkStandardchoiceValueIsActive(stdChoiceItem,issuedStatus);
		if(isValidValue)
			{
			logDebug("The valid expiration status value is '" +issuedStatus+ "'");
			return issuedStatus; 
			}
		}
	return null; 
 ...
 /**
 * Enabled standard choice item or create the new item.
 *
 * @param {string} standardChoiceName      
 * @returns null
 */
function enableStandardChoiceOrCreateItem(standardChoiceName)
{
	var rBizDomainModel = new com.accela.aa.aamain.systemConfig.RBizDomainModel() ;
	rBizDomainModel.setServiceProviderCode(aa.getServiceProviderCode()) ;
	rBizDomainModel.setBizDomain(standardChoiceName) ;
	var bizDomainBusiness= com.accela.aa.aamain.systemConfig.BizDomainBusiness() ; 
	var RBizDomainArr = bizDomainBusiness.getRBizDomain(rBizDomainModel).toArray()  ;
	if(RBizDomainArr.length > 0) 
	{
	for( var i =0 ; i< RBizDomainArr.length ; i++)
		{
		 var standardChoiceStatus = RBizDomainArr[i].getAuditStatus() //: I
		 if(standardChoiceStatus == "I")
			 {
			 RBizDomainArr[i].setAuditStatus("A") ;
			 bizDomainBusiness.editRBizDomain(RBizDomainArr[i]) ;
			logDebug("Successfully Enabled Standard Choice Name " + standardChoiceName )
			 }
		}
	}
	else
		{
		// create new standard choice item with empty value.
		rBizDomainModel.setAuditStatus("A") ; 
		rBizDomainModel.setAuditDate(new Date(aa.util.now()))
		rBizDomainModel.setAuditID("ADMIN")
		bizDomainBusiness.createRBizDomain(rBizDomainModel)
		logDebug("Successfully Created Standard Choice Item '" + standardChoiceName +"'" )
		}
	
}

/**
 * Check standard choice item value is exists and active by item name and value
 *
 * @param {string} stdChoice     
 * @param {string} stdValue    
 * @returns {boolean}
 */
function checkStandardchoiceValueIsActive(stdChoice, stdValue)
{
	var bizDomScriptResult = aa.bizDomain.getBizDomainByValue(stdChoice, stdValue);
	if (bizDomScriptResult.getSuccess()) {
		bizDomainScriptModel = bizDomScriptResult.getOutput(); 
		if(bizDomainScriptModel.getAuditStatus() == "A")
			{
			logDebug("Standard choice item '"+stdChoice+"' and value '"+stdValue+"' is exists.");
			return true; 
			}

	}
	logDebug("Standard choice item '"+stdChoice+"' and value '"+stdValue+"' does not exist.");
	return false; 
}
