...
var fuel = getFieldValue("Store Fuel", asiGroups);
var suppression = getFieldValue("Sound suppression", asiGroups);
var leaks = getFieldValue("Leaks/spill managed", asiGroups);

cancel = false;
try {
    //

    var powerSource = getASITablesRowsFromSession4ACA("POWER SOURCE(S)");
    var chrisG = true;


    for(x in powerSource) {
        if(powerSource[x]["Type of Power"] == "Generator") {
            if(suppression.toString().length() == 0 && leaks.toString().length() == 0 && (fuel != "Yes" || fuel != "No"))
                showMessage = true;
            comment("You must populate the following fields, 'Do you store fuel on-site?', 'How are leaks and spills managed?' and 'What measures are taken to supress the sound?'");
            cancel = true;
