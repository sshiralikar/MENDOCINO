    ...
    "ApplicationSubmitBefore": [
      {
        ...
        "customLists": [
          ...
          "numberLP": 1,
          "numberContacts": 1,
          "RecordAssignedtoStaffDepartment" : false
        },
        "action": {
          "validationMessage": "",
...
try {
	...
	showValidationMessage();
	logDebug("**ERROR: STDBASE_RECORD_VALIDATION Exception while verifying the rules for " + scriptSuffix + ". Error: " + ex +" line " + ex.lineNumber);
}
...
function validateRecord(rules) {
	...
	if (!isEmptyOrNull(rules.criteria.requiredLP)) {
		...
		if (!isExists) {
			validationMessage += "These License Types are required: " + rules.criteria.requiredLP + '</br>';
		}
	...
	if (!isEmptyOrNull(rules.criteria.validateLP)) {
		...
		if (isExpired) {
			validationMessage += '</br>' + "The " + lpArray[ca]["licType"] + " License is expired" + '</br>';
	...
	if (!isEmptyOrNull(rules.criteria.numberLP) && parseInt(rules.criteria.numberLP) > 0 && !isNaN(parseInt(rules.criteria.numberLP))) {
		...
		if (LPrequiredCount > 0) {
			if (parseInt(rules.criteria.numberLP) > LPrequiredCount) {
				validationMessage += '</br>' + "There should be " + rules.criteria.numberLP + " from these License Types: " + requiredLPArray + '</br>';

			}
		} else if (lbCount < rules.criteria.numberLP) {
			validationMessage += '</br>' + "There should be " + rules.criteria.numberLP + " Licenses" + '</br>';
	...
	if (!isEmptyOrNull(rules.criteria.requiredContact)) {
		...
		if (!isExists) {
			validationMessage += '</br>' + "The following contacts are required: " + requiredContactArray + '</br>';
	...
	if (!isEmptyOrNull(rules.criteria.numberContacts)) {
		...
		if (contactrequiredCount > 0) {
			if (parseInt(rules.criteria.numberContacts) > parseInt(contactrequiredCount)) {
				validationMessage += '</br>' + "There should be " + rules.criteria.numberContacts + " from these Contacts " + requiredContactArray + '</br>';

			}
		} else if (countactsCount < parseInt(rules.criteria.numberContacts)) {
			validationMessage += '</br>' + "There should be " + rules.criteria.numberContacts + " Contacts" + '</br>';
	...
	if (!isEmptyOrNull(rules.criteria.RecordAssignedtoStaffDepartment)) {
	var RecordAssignedtoStaffDepartment = rules.criteria.RecordAssignedtoStaffDepartment ; 
	if(RecordAssignedtoStaffDepartment == false )
	{
	...
	if (cdScriptObjResult.getSuccess()) 
		...
		var assignStaff = cd.getAsgnStaff() ;
		if(assignStaff == null)
			{
				validationMessage += '</br>' + "Please update the Assigned to Staff field under the Record tab, this must be completed prior to completing this workflow task" + '</br>';
			}
		}
	}
}
	...
	if (validationMessage != "") {
		mainValidationMsg += (rules.action.validationMessage == null || rules.action.validationMessage == "") ? validationMessage : rules.action.validationMessage += '</br></br>' + validationMessage;
